/* https://warmwishesflorist.com/wp-content/plugins/woo-social-login/includes/js/woo-slg-public.js */
try{
"use strict";jQuery(document).ready(function($){if($(".woo-slg-social-login-googleplus").length>0){var auth2;var googleUser={};if(WOOSlg.google_client_id!=""){var woo_slg_start_app=function(){gapi.load('auth2',function(){auth2=gapi.auth2.init({client_id:WOOSlg.google_client_id,cookiepolicy:'single_host_origin',});$('.woo-slg-social-login-googleplus').each(function(i,j){woo_slg_google_signin(j)})})};woo_slg_start_app()}else{var object=$('.woo-slg-social-login-googleplus');var errorel=$(object).parents('.woo-slg-social-container').find('.woo-slg-login-error');errorel.hide();errorel.html('');if(WOOSlg.gperror=='1'){errorel.show();errorel.html(WOOSlg.gperrormsg);return!1}}
function woo_slg_google_signin(element){var object=$(element);var errorel=$(object).parents('.woo-slg-social-container').find('.woo-slg-login-error');errorel.hide();errorel.html('');if(WOOSlg.gperror=='1'){errorel.show();errorel.html(WOOSlg.gperrormsg);return!1}
auth2.attachClickHandler(element,{},function(googleUser){var profile=googleUser.getBasicProfile();var gp_userdata={given_name:profile.getGivenName(),family_name:profile.getFamilyName(),name:profile.getName(),email:profile.getEmail(),id:profile.getId(),img_url:profile.getImageUrl(),};var woo_slg_post_data={action:'woo_slg_social_login',type:'googleplus',gp_userdata:gp_userdata,};$.ajax({url:WOOSlg.ajaxurl,type:'post',data:woo_slg_post_data,success:function(woo_slg_google_ajax_response){if(woo_slg_google_ajax_response){woo_slg_social_connect('googleplus',object)}}})},function(error){if(error.error=="popup_closed_by_user"){return!1}else{errorel.show();errorel.html(error.error);return!1}})}}
...
errorel.hide();errorel.html('');if(WOOSlg.twerror=='1'){errorel.show();errorel.html(WOOSlg.twerrormsg);return!1}else{var twitterurl=WOOSlg.tw_authurl;if(twitterurl==''){alert(WOOSlg.urlerror);return!1}
...
window.location=current_url}}else{successel.show();successel.html(response.message)}}
if(response.error){errorel.show();errorel.html(response.message)}}});return!1})});function woo_slg_social_connect(type,object){var data={action:'woo_slg_social_login',type:type};jQuery(object).parents('.woo-slg-social-container').find('.woo-slg-login-loader').show();jQuery(object).parents('.woo-slg-social-container').find('.woo-slg-social-wrap').hide();jQuery.post(WOOSlg.ajaxurl,data,function(response){jQuery(object).parents('.woo-slg-social-container').find('.woo-slg-login-loader').hide();jQuery(object).parents('.woo-slg-social-container').find('.woo-slg-social-wrap').show();var redirect_url=object.parents('.woo-slg-social-container').find('.woo-slg-redirect-url').val();if(response!=''){var result=jQuery.parseJSON(response);if(redirect_url!=''){redirect_url=removeParam(redirect_url,'code');if(WOOSlg.caching_enable=='yes'){var strexist=redirect_url.indexOf("?");if(strexist>=0){redirect_url=redirect_url+'&no-caching=1'}else{redirect_url=redirect_url+'?no-caching=1'}}
...
return isWebView}
if(typeof jQuery!=='undefined'){var targetWindow='prefer-popup';(function($){$('a[data-plugin="woo-slg"][data-action="connect"],a[data-plugin="woo-slg"][data-action="link"]').on('click',function(e){var $target=$(this),href=$target.attr('href'),success=!1;if(href.indexOf('?')!==-1){href+='&'}else{href+='?'}
...
if(NSLPopupCenter(href+'display=popup','nsl-social-connect',$target.data('popupwidth'),$target.data('popupheight'))){success=!0;e.preventDefault()}
if(!success){window.location=href;e.preventDefault()}})})(jQuery)}
...
/* https://warmwishesflorist.com/wp-content/plugins/mailchimp-for-woocommerce/public/js/mailchimp-woocommerce-checkout-gdpr.min.js */
try{
