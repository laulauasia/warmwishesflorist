...
class Session extends StorageAbstract {
    ...
    public function __construct() {
        ...
        if (defined('WOOSLG_SESSION_NAME')) {
    ...
    protected function load($createSession = false) {
        ...
        if ($this->sessionId === null) {
            if (isset($_COOKIE[$this->sessionName])) {
                $this->sessionId = 'wooslg_persistent_' . md5(SECURE_AUTH_KEY . $_COOKIE[$this->sessionName]);
                ...
                $this->sessionId = 'wooslg_persistent_' . md5(SECURE_AUTH_KEY . $unique);
